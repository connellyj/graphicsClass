...
void meshSetVaryVertex(renRenderer *ren, meshMesh *mesh, int vert, double vary[]) {
	...
	if (0 <= vert && vert < mesh->vertNum)
		for (k = 0; k < ren->varyDim; k += 1)
			meshTransformedVertices[ren->varyDim * vert + k] = vary[k];
}

void transformVertices(meshMesh *mesh, renRenderer *ren, double unif[]) {
    for(int i = 0; i < mesh->vertNum; i++) {
        double v[ren->varyDim];
        ren->transformVertex(ren, unif, meshGetVertexPointer(mesh, i), v);
        meshSetVaryVertex(ren, mesh, i, v);
    }
}
