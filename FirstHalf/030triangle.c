...
void getChi(double a[2], double x[2],  double chi[3], 
            double alpha[3], double beta[3], double gamma[3], double m[2][2]){
    double xMinusA[2];
...
void permutePoints(double a[2], double b[2], double c[2], double points[3][2], 
                   double alpha[3], double beta[3], double gamma[3], double colors[3][3]) {
    if(c[0] <= a[0] && c[0] <= b[0]) {
        points[0][0] = c[0];
        points[0][1] = c[1];
        points[1][0] = a[0];
...
void triRender(double a[2], double b[2], double c[2], double rgb[3],
    ...
    if(points[1][0] >= points[2][0]) {
        ...
        for(x = ceil(points[0][0]); x <= floor(points[2][0]); ++x) {
            ...
            fillColumn(x, minY, maxY, rgb, points[0], colors[0], colors[1], colors[2], m);
        }
