...
double varyVert[renVERTNUMBOUND * renVARYDIMBOUND];
...
/* Sets the vertth vertex to have attributes vary. */
void meshSetVaryVertex(renRenderer *ren, meshMesh *mesh, int vert, double vary[]) {
	int k;
	if (0 <= vert && vert < mesh->vertNum)
		for (k = 0; k < ren->varyDim; k += 1)
			varyVert[ren->varyDim * vert + k] = vary[k];
}
...
void meshRender(meshMesh *mesh, renRenderer *ren, double unif[], 
    ...
    for(int i = 0; i < mesh->triNum; i++){
        ...
        transformVertex(ren, unif, meshGetVertexPointer(mesh, tempTri[0]), a);
        transformVertex(ren, unif, meshGetVertexPointer(mesh, tempTri[1]), b);
        transformVertex(ren, unif, meshGetVertexPointer(mesh, tempTri[2]), c);
        meshSetVaryVertex(ren, mesh, tempTri[0], a);
        meshSetVaryVertex(ren, mesh, tempTri[1], b);
        meshSetVaryVertex(ren, mesh, tempTri[2], c);
