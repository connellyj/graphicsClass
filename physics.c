...
typedef struct {
    ...
    double bounceVel; // minimum incoming velocity required for an object to bounce
    double mu;
    int mode;
...
void pMat33FromPhysics(float m[3][3], const dReal *p) {
    int offset = 0;
    for(int i = 0; i < 3; i++) {
        m[i][0] = p[offset];
        m[i][1] = p[1 + offset];
...
void pVec3FromPhysics(float v[3], const dReal *p) {
    for(int i = 0; i < 3; i++) {
        v[i] = (float) p[i];
    }
}
