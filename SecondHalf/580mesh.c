/* OpenGL methods filled in by Julia Connelly and Kerim Celik, 02/24/2017 */
...
void meshGLVAOInitialize(meshGLMesh *meshGL, GLuint index, GLint attrLocs[]){
    ...
    for(int i = 0; i < meshGL->attrNum; i++) {
        ...
        buffOffset += (meshGL->attrDims[i] * sizeof(GLdouble));
    }
...
void meshGLRender(meshGLMesh *meshGL, GLint index) {
	/* Draw the triangles */
    glBindVertexArray(meshGL->vaos[index]);
	glDrawElements(GL_TRIANGLES, (meshGL->triNum) * 3, GL_UNSIGNED_INT, BUFFER_OFFSET(0));
    glBindVertexArray(0);
	/* Why don't we ever disable these anymore? */
